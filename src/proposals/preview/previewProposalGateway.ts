...
export class PreviewProposalGateway implements ProposalGateway {
  ...
  async assignProposalReviewer(
    ...
    const assignment = {
      ...
      proposalId,
      reviewerUserId,
      reviewerName: 'Revisor Designado',
  ...
  async startProposalReview(
    ...
    if (!canTransitionProposalStatus(existing.status, 'under_review')) {
      throw new ProposalDomainError('OPERATION_NOT_ALLOWED', `Transição inválida para under_review.`);
    }
  ...
  async requestProposalChanges(
    ...
    if (!canTransitionProposalStatus(existing.status, 'changes_requested')) {
      throw new ProposalDomainError('OPERATION_NOT_ALLOWED', `Transição inválida para changes_requested.`);
    }
  ...
  async approveProposal(
    ...
    if (!canTransitionProposalStatus(existing.status, 'approved')) {
      throw new ProposalDomainError('OPERATION_NOT_ALLOWED', `Transição inválida para approved.`);
    }
    ...
    const updated: Proposal = {
      ...
      notes: notes ? `${existing.notes || ''}\n[Aprovação]: ${notes}`.trim() : existing.notes,
      updatedAt: now,
      version: existing.version + 1,
    };
    orgMap.set(proposalId, updated);
  ...
  async rejectProposal(
    ...
    if (!canTransitionProposalStatus(existing.status, 'rejected')) {
      throw new ProposalDomainError('OPERATION_NOT_ALLOWED', `Transição inválida para rejected.`);
    }
  ...
  async recordProposalDecision(
    ...
    if (existing.status !== 'presented') {
      throw new ProposalDomainError('NOT_PRESENTED', 'Decisão só pode ser registrada em propostas apresentadas.');
    }
    ...
    const updated: Proposal = {
      ...
      decisionRecord: {
        ...
        notes: input.notes,
        disclaimerText: 'Registro declaratório formal de decisão.',
      },
      updatedAt: now,
      version: existing.version + 1,
    };
  ...
  async expireDueProposals(organizationId: string): Promise<number> {
    const orgMap = this.getOrgProposalsMap(organizationId);
    const nowTime = Date.now();
    let count = 0;
    for (const [id, proposal] of orgMap.entries()) {
      if (proposal.status === 'presented' && new Date(proposal.expiresAt).getTime() <= nowTime) {
        const updated: Proposal = {
          ...proposal,
          status: 'expired',
