        ...
        {visibleChecklists.map((checklist) => (
          <article key={checklist.id} className={`${DOCUMENT_THEME.surface} overflow-hidden`}>
            ...
            <ul className="space-y-4 p-5 sm:p-6">{checklist.items.map((item) => {
              ...
              return <li key={item.id} className={`rounded-2xl border p-4 sm:p-5 ${stateClass(item.effectiveState ?? item.state)}`}><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-bold text-[#0B3D2E]">{item.title}</h3><p className="mt-1 text-sm text-[#0B3D2E]/70">{DOCUMENT_CATEGORY_LABELS[item.category]} · {item.required ? 'Obrigatório' : 'Opcional'} · {formatDate(item.dueOn)}</p>{item.effectiveStateReason ? <p className="mt-1 text-sm font-semibold text-[#0B3D2E]">{item.effectiveStateReason}</p> : null}</div><span className={DOCUMENT_THEME.badge}>{PROPOSAL_CHECKLIST_STATE_LABELS[item.effectiveState ?? item.state]}</span></div>{item.linkedDocumentId ? <button type="button" className={`${DOCUMENT_THEME.buttonSecondary} mt-3`} onClick={() => navigate(getDocumentReferencePath(item.linkedDocumentId!))}><FileCheck2 className="h-4 w-4" aria-hidden="true" /> Abrir documento recebido</button> : null}
