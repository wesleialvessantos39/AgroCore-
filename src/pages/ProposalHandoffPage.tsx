...
import { Proposal, ProposalOperationalHandoff } from '../types/proposals';
...
export const ProposalHandoffPage: React.FC = () => {
  ...
  const { getProposalById, getProposalHandoff, prepareProposalHandoff } = useProposals();
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    const [currentProposal, currentHandoff] = await Promise.all([
    ...
    setLoading(false);
  }, [getProposalById, getProposalHandoff, proposalId]);
  ...
  const handlePrepare = async () => {
    ...
    if (result.success && result.data) {
      ...
      setFeedback('Encaminhamento operacional preparado com integridade verificada.');
    } else {
      setFeedback(result.error ?? 'Não foi possível preparar o encaminhamento.');
    }
    setBusy(false);
  };
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6" id="page-proposal-handoff">
      <header>
        <Link className={PROPOSAL_THEME.btnMutedSmall} to={getProposalDetailPath(proposal.id)}>
          <ArrowLeft className="h-4 w-4" aria-hidden="true" /> Voltar à proposta
        </Link>
        <p className={`mt-4 text-sm font-semibold ${PROPOSAL_THEME.textSecondary}`}>OE‑005.005 · {proposal.proposalNumber}</p>
      ...
      {feedback && <div className={`${PROPOSAL_THEME.surfaceSoft} ${PROPOSAL_THEME.border} rounded-xl border p-4 ${PROPOSAL_THEME.textPrimary}`} role="status" aria-live="polite">{feedback}</div>}
