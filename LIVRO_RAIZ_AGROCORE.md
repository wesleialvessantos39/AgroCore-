...
- **Status Geral:** OE-007.001 a OE-007.005 estão implementadas no código. A OE-007.003, a OE-007.004 e as migrations da **OE-007.005 — Conclusão e Relatório** estão aplicadas no Supabase AgroCore. A auditoria corrigiu o bypass de máquina de estados, o snapshot de evidência para usar o vínculo canônico por imóvel, a validação/normalização server-side de pendências, o bypass por versão esperada nula e a possibilidade de combinar transição de estado com alteração silenciosa de planejamento. A conclusão é atômica: visita em execução + responsável autenticado + formulário submetido + versão 1 do relatório final. O GitHub Actions #101 existe, mas falha antes de executar qualquer step (`steps=[]`, log indisponível/`BlobNotFound`); o status externo Vercel continua `failure`. Portanto não é declarada homologação integral de produção. A próxima ordem arquitetural é a **OE-007.006 — Integração com agenda, propostas e frota**.
...
- **Entregas da OE-007.005 — Conclusão e Relatório:**
  ...
  - **Supabase aplicado:** as migrations `oe_007_005_visit_completion_reports` (`20260903021023`) e `oe_007_005_report_hardening` (`20260903022428`) estão registradas no projeto Supabase AgroCore; tabela, RLS e RPCs foram verificados remotamente. O hardening adiciona índice para a FK de autoria e limita IDs internos de pendência a 120 caracteres. Os RPCs usam `SECURITY DEFINER` com `search_path` vazio e checagens internas explícitas de autorização.
  - **Cobertura automatizada definida:** `scripts/test-field-visits-completion.ts` adiciona 27 verificações estruturais específicas da OE-007.005 e foi incorporado ao `test:module-007`. A fundação também passou a concluir visita pelo novo serviço de relatório.
  - **Situação de CI/deploy:** o GitHub Actions existe e a execução #101 do AgroCore CI foi associada ao commit desta revisão, porém o job falhou antes de iniciar qualquer step (`steps=[]`) e a tentativa de leitura do log retornou `BlobNotFound`; assim não há evidência de falha de TypeScript/teste dentro do workflow. O status externo Vercel continua `failure` e o projeto AgroCore não está disponível na conexão Vercel desta sessão. Portanto as 27 provas estão versionadas e foram confrontadas estruturalmente com a `main`, mas a execução remota integral não é declarada.
  - **Decisão objetiva:** OE-007.005 está implementada no código e aplicada no Supabase no escopo desta ordem. A homologação integral de produção permanece pendente dos gates externos já citados. A próxima OE é a OE-007.006.
...
| `npm run test:field-visits-evidence` | Valida a OE-007.004 com 48 provas de fonte canônica por imóvel, sincronização Laudo ↔ Visita ↔ Cadastro, fotos, GPS, solicitações ao captador, RBAC, RLS, Storage e produção deny-by-default |
| `npm run test:field-visits-completion` | Valida a OE-007.005 com 27 verificações de conclusão atômica, relatório versionado, snapshot canônico, pendências, concorrência, RLS/RBAC, revisão imutável, integridade de transições e bloqueio de conclusão genérica |
| `npm run test:field-visits-accessibility` | Executa 22 verificações estruturais de responsividade/acessibilidade, incluindo larguras 320/390/430/720/768/1024/1366/1440, alvos de 44 px, foco, ARIA, semântica, formulário e evidências |
...
| `npm run test:module-007` | Gate consolidado do Módulo 007 até OE-007.005: 35 provas da OE-007.001 + 37 da OE-007.002 + 51 da OE-007.003 + 48 da OE-007.004 + 27 da OE-007.005 = 198 provas definidas, além de 22 verificações de acessibilidade/responsividade, tema e textos públicos. A execução remota integral do commit atual permanece separada do número de provas definidas. |
...
| `npm run build` | Gate de produção: invariantes de release + TypeScript estrito + `test:module-006` + `test:module-007` + Vite + geração de SW + verificação de vazamentos |
...
4. **Módulo 007 — Visitas, Vistorias e Operação em Campo:** OE-007.001 a OE-007.005 estão implementadas; OE-007.003/004/005 possuem persistência aplicada no Supabase AgroCore. A OE-007.005 introduziu conclusão atômica e relatório final versionado e recebeu hardenings de máquina de estados, snapshot canônico, concorrência, pendências e auditoria server-side. O código atual define 198 provas específicas das OE-007.001–005 e 22 verificações adicionais de acessibilidade/responsividade. O AgroCore CI possui execução associada ao commit, mas o job encerra antes de qualquer step e sem log utilizável; o Vercel continua em falha. Por isso a homologação integral de produção não é declarada. Evidência física em aparelhos e condições reais de campo continua reservada à OE-007.007. Próxima ordem: OE-007.006.