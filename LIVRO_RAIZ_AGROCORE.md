...
- **Status Geral:** OE-007.001 e OE-007.002 permanecem implementadas e revisadas. A **OE-007.003 — Formulário de Campo** está implementada e aplicada no Supabase AgroCore. A **OE-007.004 — Fotos e Geolocalização** está implementada e revisada até R4: a fonte canônica é o imóvel; Laudo e Visita/Vistoria apontam para o mesmo conjunto de evidências; o fluxo Projetista → Captador para pendências cadastrais está persistido no Supabase; e o vínculo Cliente ↔ Captador foi ligado ao backend real em produção. O Vercel do commit atual permanece em falha e o AgroCore CI continua encerrando antes de executar qualquer etapa (`steps=[]`), portanto não é declarada homologação integral de produção da OE-007.004.
