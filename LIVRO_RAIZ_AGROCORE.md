...
- **Status Geral:** módulo iniciado. A **OE-008.001 — Modelo de Tarefas e Compromissos** está implementada, persistida no Supabase AgroCore e coberta pelo gate de produção. As OE-008.002 a OE-008.007 permanecem futuras e não foram antecipadas. O Vercel aprovou o gate integral no commit `a1fba4beea09ad5881a387934cc2627cd51002ad`, executando contrato de ambiente, invariantes de release, TypeScript estrito e regressões dos Módulos 001 a 008 antes do build Vite, Service Worker e verificação de vazamentos. O AgroCore CI/GitHub Actions continua encerrando antes do primeiro step, pendência externa já isolada e sem evidência de falha executada dentro do workflow.
...
- **Entregas da OE-008.001 — Modelo de Tarefas e Compromissos:**
  ...
  - **Escopo preservado:** OE-008.001 não implementa visão pessoal/equipe/calendário completo (OE-008.002), responsáveis/participantes/conclusão/reabertura/cancelamento (OE-008.003), materialização de ocorrências recorrentes e regras derivadas (OE-008.004), central de notificações (OE-008.005) nem canais externos/escalonamento (OE-008.006).
  - **Suíte:** `scripts/test-schedule-foundation.ts` contém **66 provas** de domínio, RBAC, isolamento, RLS, idempotência, concorrência, retry, DST/fuso, recorrência, origem, rota e persistência; `scripts/test-schedule-accessibility.ts` contém **18 verificações** de acessibilidade/responsividade; `scripts/test-schedule-theme.js` protege a paleta oficial; `scripts/test-module-008.js` agrega o gate inicial do módulo.
  ...
  - **Decisão objetiva:** **OE-008.001 concluída em código, banco remoto, RLS/grants/RPCs e gate automatizado de produção.** A próxima ordem permitida é OE-008.002, sem reabrir nem antecipar os fluxos reservados às OEs 008.003–008.006.
...
| `npm run test:schedule-accessibility` | Executa 18 verificações estruturais de responsividade/acessibilidade da Agenda, incluindo controles móveis, semântica, foco e recorrência semanal. |
| `npm run test:schedule-theme` | Audita a identidade visual oficial da Agenda e impede famílias de cores não homologadas/códigos internos. |
| `npm run test:module-008` | Gate inicial do Módulo 008; nesta fase agrega a OE-008.001 e será ampliado incrementalmente pelas próximas ordens. |
...
5. **Módulo 008 — Agenda Corporativa, Tarefas, Prazos e Notificações:** OE-008.001 está concluída com três migrations aplicadas no Supabase, 66 provas de fundação, 18 verificações de acessibilidade/responsividade, auditoria de tema, RLS/grants/RPCs inspecionados, lockfile reproduzível restaurado e gate integral Vercel SUCCESS no commit `a1fba4beea09ad5881a387934cc2627cd51002ad`. A próxima ordem é **OE-008.002 — Listas e Agenda**, limitada a visão pessoal/equipe, calendário e lista móvel acessível; atribuição/colaboração, ocorrências, notificações e canais externos permanecem reservados às OEs posteriores.