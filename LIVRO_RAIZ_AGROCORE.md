...
- **Status Geral:** módulo iniciado. A **OE-008.001 — Modelo de Tarefas e Compromissos** está implementada, persistida no Supabase AgroCore e coberta pelo gate de produção. As OE-008.002 a OE-008.007 permanecem futuras e não foram antecipadas. O Vercel aprovou o `npm run build` no commit `e3d21ae970542447d68fc23f094b47a0b4e9b5d9`, já incluindo TypeScript estrito, invariantes de release e `test:module-008`. O AgroCore CI/GitHub Actions continua encerrando antes do primeiro step, pendência externa já isolada e sem evidência de falha executada dentro do workflow.
...
- **Entregas da OE-008.001 — Modelo de Tarefas e Compromissos:**
  ...
  - **Suíte:** `scripts/test-schedule-foundation.ts` contém **62 provas** de domínio, RBAC, isolamento, RLS, idempotência, concorrência, retry, DST/fuso, recorrência, origem, rota e persistência; `scripts/test-schedule-accessibility.ts` contém **18 verificações** de acessibilidade/responsividade; `scripts/test-schedule-theme.js` protege a paleta oficial; `scripts/test-module-008.js` agrega o gate inicial do módulo.
  - **Gate externo observado:** Vercel **SUCCESS** no commit `e3d21ae970542447d68fc23f094b47a0b4e9b5d9`, executando o `npm run build` que já inclui `test:module-008`.
...
| `npm run test:schedule-foundation` | Valida a OE-008.001 com 62 provas de tarefas/compromissos, sete perfis, tenant/IDOR, origem, recorrência, UTC/DST, RLS, concorrência, idempotência, retry e persistência. |
...
| `npm run build` | Gate de produção: invariantes de release + TypeScript estrito + gates dos módulos consolidados, incluindo `test:module-006`, `test:module-007` e `test:module-008` + Vite + geração de SW + verificação de vazamentos |
...
5. **Módulo 008 — Agenda Corporativa, Tarefas, Prazos e Notificações:** OE-008.001 está concluída com três migrations aplicadas no Supabase, 62 provas de fundação, 18 verificações de acessibilidade/responsividade, auditoria de tema, RLS/grants/RPCs inspecionados e Vercel SUCCESS no commit `e3d21ae970542447d68fc23f094b47a0b4e9b5d9`. A próxima ordem é **OE-008.002 — Listas e Agenda**, limitada a visão pessoal/equipe, calendário e lista móvel acessível; atribuição/colaboração, ocorrências, notificações e canais externos permanecem reservados às OEs posteriores.