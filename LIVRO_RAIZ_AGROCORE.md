...
- **Status Geral:** OE-007.001 e OE-007.002 permanecem implementadas e revisadas. A **OE-007.003 — Formulário de Campo** está implementada e aplicada no Supabase AgroCore. A **OE-007.004 — Fotos e Geolocalização** está implementada e revisada até R5: a fonte canônica é o imóvel; Laudo e Visita/Vistoria apontam para o mesmo conjunto de evidências; o fluxo Projetista → Captador para pendências cadastrais está persistido no Supabase; a R5 valida a origem real da solicitação contra o mesmo cliente/imóvel e oferece acesso direto à área canônica no cadastro do cliente; e o vínculo Cliente ↔ Captador foi ligado ao backend real em produção. O Vercel do commit atual permanece em falha e o AgroCore CI continua encerrando antes de executar qualquer etapa (`steps=[]`), portanto não é declarada homologação integral de produção da OE-007.004.
...
- **Entregas da OE-007.004 — Fotos e Geolocalização Canônicas do Imóvel:**
  ...
  - **Gate atual:** código e banco da correção estão implementados; OE-007.005 permanece deliberadamente não iniciada nesta revisão.
...
| `npm run test:module-007` | Gate consolidado do Módulo 007 até OE-007.004: 35 provas da OE-007.001 + 37 da OE-007.002 + 51 da OE-007.003 + 48 da OE-007.004 = 171 provas definidas, além de 22 verificações de acessibilidade/responsividade, tema e textos públicos. A execução remota integral do commit atual permanece separada do número de provas definidas. |
...
4. **Módulo 007 — Visitas, Vistorias e Operação em Campo:** OE-007.001 e OE-007.002 permanecem revisadas; OE-007.003 está implementada/aplicada no Supabase; OE-007.004 está implementada até R4 com evidência única por imóvel, sincronização Laudo ↔ Visita/Vistoria ↔ Cadastro do Cliente, fila Projetista → Captador e gateway Cliente ↔ Captador persistente. O código atual define 171 provas específicas das OE-007.001–004 e 22 verificações adicionais de acessibilidade/responsividade. O Vercel atual continua em falha sem log acessível ao conector e o GitHub Actions encerra antes das etapas; por isso OE-007.005 não foi iniciada nesta revisão e a homologação integral de produção da OE-007.004 não é declarada. Evidência física em aparelhos e condições reais de campo continua reservada à OE-007.007.