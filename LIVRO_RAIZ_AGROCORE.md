...
- **Status Geral:** módulo implementado até **OE-008.002 — Listas e Agenda**. A OE-008.001 permanece concluída e a OE-008.002 adiciona visão pessoal/equipe, calendário mensal e lista móvel acessível sem criar fonte canônica paralela. A migration `20260903190345_oe_008_002_schedule_view_indexes.sql` está aplicada no Supabase AgroCore. As OE-008.003 a OE-008.007 permanecem futuras e não foram antecipadas. O último gate integral efetivamente executado pelo Vercel continua sendo o SUCCESS do commit `a1fba4beea09ad5881a387934cc2627cd51002ad`, anterior à OE-008.002; os commits atuais da OE-008.002 não recebem novo build porque a conta Vercel passou a responder `Deployment rate limited — retry in 24 hours`. O AgroCore CI/GitHub Actions continua encerrando antes do primeiro step, portanto o fechamento da OE-008.002 separa explicitamente implementação/persistência e inspeção estrutural do gate externo ainda não executado.

- **Entregas da OE-008.001 — Modelo de Tarefas e Compromissos:**
  - **Domínio tipado:** `CorporateTask` e `CalendarAppointment` compartilham organização, título, descrição, prioridade `low/medium/high/urgent`, estados `pending/in_progress/blocked/completed/cancelled`, fuso IANA, recorrência declarativa, origem, autoria e versão otimista. Tarefa usa prazo opcional; compromisso exige início/fim coerentes.
  - **Fonte autoritativa:** `public.schedule_items` é a fonte persistente do modelo inicial. Não foram criados cadastros paralelos de cliente, imóvel, proposta, visita ou usuário. Origem integrada é representada somente por referências estáveis `source_domain/source_id/source_version/source_event_key`; o registro pertencente ao domínio de origem não pode ser editado pela mutação manual.
  - **Eventos predecessores:** o Módulo 007 permanece produtor dos eventos estáveis `calendar.visit_sync_requested` e `calendar.visit_release_requested`. A OE-008.001 preserva o contrato de origem, mas não materializa ainda os fluxos de conclusão/cancelamento/ocorrências que pertencem às OEs posteriores.
  - **Recorrência como contrato:** frequências `none/daily/weekly/monthly/yearly`, intervalo de 1 a 365, término opcional e dias da semana somente para recorrência semanal. Após a revisão R2, recorrência semanal exige ao menos um dia explícito, recusando listas vazias, duplicadas ou inválidas.
  - **UTC e fusos:** datas são normalizadas em UTC e o fuso IANA é preservado. A camada de domínio recusa fuso inexistente, horário local inexistente em salto de DST e horário ambíguo em retorno de DST.
  - **RBAC positivo/negativo:** `owner`, `company_admin` e `manager` possuem `schedule:view` + `schedule:manage`; `project_designer` e `capturer` possuem apenas `schedule:view`; `finance` e `platform_super_admin` não recebem acesso organizacional à Agenda por herança. A mesma regra é aplicada no serviço, rota/navegação e backend remoto.
  - **Tenant/IDOR:** todas as consultas e RPCs são vinculadas a `organization_id` + membership ativa. IDs de outro tenant não são enumerados pela interface e as policies RLS usam autorização organizacional.
  - **Persistência remota:** migrations aplicadas no Supabase AgroCore:
    - `20260903153944_oe_008_001_schedule_model.sql` — tabelas `schedule_items` e `schedule_item_audit`, RLS, grants, índices e RPCs iniciais.
    - `20260903155402_oe_008_001_schedule_fk_hardening.sql` — índice direto da FK `schedule_item_audit.schedule_item_id`.
    - `20260903175453_oe_008_001_command_idempotency_hardening.sql` — recibos privados de comandos, SHA-256, idempotência do update e recorrência semanal explícita.
  - **RLS/grants:** inspeção remota confirmou RLS ativa em `schedule_items` e `schedule_item_audit`; `anon` não possui SELECT; `authenticated` possui somente leitura direta e não possui INSERT/UPDATE/DELETE. Escritas entram pelas RPCs autorizadas.
  - **RPCs:** `agrocore_create_schedule_item(uuid,jsonb,text)` e `agrocore_update_schedule_item(uuid,uuid,integer,text,jsonb,text)` estão como `SECURITY DEFINER`, `search_path` vazio, sem EXECUTE para `anon` e com EXECUTE para `authenticated` como fronteira intencional da aplicação. Os avisos do advisor sobre `SECURITY DEFINER` são esperados e mitigados por autorização interna, validação e `search_path` fechado.
  - **Idempotência/retry:** criação e edição usam command key; os recibos ficam em `agrocore_private.schedule_item_command_receipts`, sem SELECT/INSERT para `authenticated` ou `anon`, e fingerprints SHA-256. O gateway remoto executa no máximo três tentativas (`0/200/600 ms`) somente para falhas transitórias de rede/serviço; erros de domínio não são repetidos. Replays com a mesma chave convergem sem duplicar auditoria ou incremento de versão.
  - **Concorrência:** edição exige `expectedVersion`, usa advisory lock e row lock no backend. Versão obsoleta é recusada quando se trata de um novo comando; replay do mesmo comando idempotente converge pelo recibo.
  - **Auditoria:** `schedule_item_audit` é append-only para criação/edição, com ator, versão, campos alterados, data e motivo. A escrita direta autenticada permanece revogada.
  - **Performance:** após R1/R2, o advisor do Supabase não aponta FK não indexada no escopo da Agenda. Os avisos restantes são apenas `unused_index` em tabelas novas e ainda sem uso significativo.
  - **Interface:** rota protegida `/agenda`, item de navegação condicionado a `schedule:view`, estado vazio real, formulário responsivo para tarefa/compromisso, prioridade, fuso, datas e recorrência. Recorrência semanal usa `fieldset/legend`, checkboxes e bloqueia o envio sem dia selecionado. Nenhum código interno de OE é renderizado.
  - **Produção deny-by-default:** o factory de produção usa `SupabaseScheduleGateway` quando a configuração real existe e `UnavailableScheduleGateway` quando não existe; o gateway preview permanece isolado de produção. Nenhum dado de negócio é persistido em localStorage/IndexedDB nesta ordem.
  - **Escopo preservado:** OE-008.001 não implementa responsáveis/participantes/conclusão/reabertura/cancelamento (OE-008.003), materialização de ocorrências recorrentes e regras derivadas (OE-008.004), central de notificações (OE-008.005) nem canais externos/escalonamento (OE-008.006). A visão pessoal/equipe e o calendário passaram a pertencer à OE-008.002 implementada abaixo.
  - **Suíte de fundação:** `scripts/test-schedule-foundation.ts` mantém **66 provas** de domínio, RBAC, isolamento, RLS, idempotência, concorrência, retry, DST/fuso, recorrência, origem, rota e persistência. A suíte compartilhada de acessibilidade foi ampliada pela OE-008.002 de 18 para **25 verificações** atuais; `scripts/test-schedule-theme.js` continua protegendo a paleta oficial.
  - **Reprodutibilidade do CI/build:** o `package-lock.json` npm v3, removido anteriormente do repositório, foi restaurado a partir de uma versão histórica cuja raiz de dependências coincide exatamente com o `package.json` atual. O `test:environment-contract` voltou a validar sincronismo package/lock, ausência de solicitação de chaves/secrets e uso de `npm ci --no-audit --no-fund` no workflow.
  - **Gate externo observado:** Vercel **SUCCESS** no commit `a1fba4beea09ad5881a387934cc2627cd51002ad`. O `npm run build` executou `test:environment-contract`, invariantes, `tsc --noEmit`, `test:module-001`, `test:module-002`, `test:module-003`, `test:module-004`, `test:module-005`, `test:module-006`, `test:module-007`, `test:module-008`, Vite, geração do Service Worker e verificação de vazamentos.
  - **Decisão objetiva:** **OE-008.001 concluída em código, banco remoto, RLS/grants/RPCs e gate automatizado de produção.** Seu contrato permanece como fundação da OE-008.002.

- **Entregas da OE-008.002 — Listas e Agenda:**
  - **Escopo executado:** a Agenda agora oferece **Minha agenda**, **Equipe**, **Lista** e **Calendário**, exatamente dentro do objetivo “visão pessoal/equipe, calendário e lista móvel acessível”. Não foi criada tabela paralela de agenda e `public.schedule_items` continua sendo a fonte autoritativa.
  - **Semântica da visão pessoal:** enquanto responsáveis e participantes ainda pertencem à OE-008.003, “Minha agenda” é definida de forma não ambígua como registros cujo `created_by_user_id` corresponde ao usuário autenticado. Essa regra não inventa atribuição nem transforma autoria em responsabilidade. “Equipe” mantém todos os registros da organização que o RBAC atual já permite consultar.
  - **Contrato tipado:** `ScheduleViewScope = 'personal' | 'team'` integra `ScheduleItemListFilters`. A fronteira de domínio normaliza `kind`, `status` e `viewScope`; valores inválidos são recusados como `INVALID_INPUT` antes do gateway. Chamadas de serviço sem `viewScope` preservam o comportamento de equipe; a interface inicia explicitamente em `personal`.
  - **Tenant/IDOR:** todas as listas continuam condicionadas a `organization_id`; no gateway Supabase a visão pessoal acrescenta filtro server-side por `created_by_user_id`. O RLS remoto de `schedule_items` permanece ativo com a policy organizacional já homologada. A visão de equipe não amplia perfis nem ignora a autorização existente.
  - **RBAC preservado:** `owner`, `company_admin` e `manager` mantêm consulta/gestão; `project_designer` e `capturer` continuam somente leitura; `finance` e `platform_super_admin` continuam sem herdar agenda privada. A OE-008.002 não criou nova permissão para contornar a matriz da OE-008.001.
  - **Calendário determinístico:** `src/schedule/calendar.ts` fornece chave de data no fuso da visualização, navegação mensal, rótulo mensal e grade fixa de 42 células. Tarefa usa `dueAt`; compromisso usa `startsAt`; tarefa sem prazo permanece em seção própria.
  - **Recorrência sem antecipação:** o calendário mostra somente registros persistidos. Uma definição recorrente não é expandida artificialmente em ocorrências futuras, preservando a materialização idempotente para OE-008.004.
  - **Interface desktop:** calendário mensal usa `role="grid"`, `role="columnheader"` e `role="gridcell"`, navegação anterior/hoje/próximo e filtros de tipo/situação. Calendário vazio permanece navegável em vez de desaparecer.
  - **Interface móvel:** abaixo do breakpoint `md`, a grade mensal é substituída por lista cronológica agrupada por dia, evitando tabela/grade horizontal comprimida. Controles principais mantêm alvo mínimo de 44 px, foco visível e estados `aria-pressed`.
  - **Estado e concorrência de leitura:** troca de escopo/filtro continua usando o `AbortController` e a sequência de requisições do `ScheduleContext`, descartando respostas obsoletas após troca de organização ou filtros. Nenhuma escrita nova foi introduzida nesta ordem; idempotência/concorrência das mutações permanecem sob o contrato da OE-008.001.
  - **Persistência remota:** migration aplicada `20260903190345_oe_008_002_schedule_view_indexes.sql`, contendo apenas três índices de leitura: `schedule_items_org_creator_kind_status_idx`, `schedule_items_org_creator_due_idx` e `schedule_items_org_creator_start_idx`. A migration não cria tabela, não duplica entidade e mantém `organization_id` como primeiro componente dos índices.
  - **Evidência Supabase:** inspeção remota confirmou os três índices, RLS ativa em `schedule_items`, policy de SELECT organizacional intacta e **0 registros** na tabela no momento da verificação; nenhum dado fictício foi inserido para demonstrar a interface. Os advisors mostram apenas `unused_index` INFO para índices novos/sem tráfego e os WARNs já conhecidos das RPCs `SECURITY DEFINER` da OE-008.001.
  - **Escopo futuro preservado:** não foram adicionados `responsibleUserId`, participantes, atribuição, conclusão, reabertura, cancelamento, geração de ocorrências, central de notificações nem canais externos. Esses itens continuam reservados às OEs 008.003–008.006.
  - **Testes definidos:** `scripts/test-schedule-views.ts` contém **41 provas sequenciais** cobrindo visão pessoal/equipe, RBAC positivo/negativo, tenant, filtros válidos/inválidos, fuso, calendário, tarefas sem data, não materialização de recorrência, desktop/mobile, acessibilidade, paleta e migration. A fundação continua com **66 provas**, totalizando **107 provas específicas** do Módulo 008 até OE-008.002, além de **25 verificações** atuais de acessibilidade/responsividade e auditoria de tema.
  - **Gate integrado:** `scripts/test-module-008.js` executa fundação, listas/agenda, acessibilidade e tema; `scripts/verify-release-invariants.js` trava a presença da suíte, do painel de navegação e da migration. O `npm run build` continua configurado para executar regressões dos Módulos 001 a 008.
  - **Limite de homologação externa:** a inspeção estrutural do HEAD confirmou 66/66 definições sequenciais da fundação, 41/41 definições sequenciais da OE-008.002 e 25/25 verificações sequenciais de acessibilidade, além dos invariantes de arquitetura verificados diretamente no repositório. **Não é declarado que o TypeScript/build atual foi executado externamente**, porque o Vercel está rate-limited e o GitHub Actions continua sem iniciar steps.
  - **Decisão objetiva:** **OE-008.002 implementada em código e persistência remota, com RLS/índices e escopo arquitetural verificados.** A execução externa do gate atual permanece pendente exclusivamente por indisponibilidade dos runners observados; a próxima ordem arquitetural é OE-008.003, mas deve ser iniciada somente mantendo explícita essa pendência de gate.
