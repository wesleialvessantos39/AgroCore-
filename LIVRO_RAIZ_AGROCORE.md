...
- **Entregas da OE-006.007 — Segurança e Homologação Documental (revisão contra Plano Mestre e Relatório Consolidado):**
  ...
  - **Evidência Remota:** o único projeto Supabase visível nesta sessão é `sitejuceliasantana`, que não pertence ao AgroCore. Nenhuma migração, política, bucket ou Edge Function do AgroCore foi aplicada nesse projeto errado. O fechamento remoto aguarda conexão do Supabase correto e verificação de migrations/RLS/Storage/function no ambiente alvo.
...
| `npm run test:field-visits-accessibility` | Executa 12 verificações estruturais de responsividade/acessibilidade: estratégia 320/390/430/720/768/1024/1366/1440, alvos de 44 px, foco, ARIA, semântica e prevenção de overflow fixo |
...
| `npm run test:module-007` | Homologação consolidada do Módulo 007 até OE-007.002: 35 provas da fundação + 37 de agenda/preparação + 12 verificações estruturais de acessibilidade/responsividade, tema e textos públicos |
...
3. **Módulo 006 — Gestão Documental:** OE-006.007 revisada contra o Plano Mestre e Relatório Consolidado, com 29 provas de segurança e 159 provas de código no total executadas no gate de produção Vercel. O fechamento de infraestrutura continua pendente porque o Supabase AgroCore correto não está conectado; migrações, RLS, Storage e `document-share` não devem ser aplicados em outro projeto. O GitHub Actions continua encerrando com zero `steps`.
4. **Módulo 007 — Visitas, Vistorias e Operação em Campo:** OE-007.001 e OE-007.002 revisadas contra o Plano Mestre e Relatório Consolidado, com 72 provas comportamentais/estruturais aprovadas e 12 verificações adicionais de acessibilidade/responsividade no gate de produção. OE-007.003 não foi iniciada. A continuidade depende da decisão operacional sobre a pendência remota do Módulo 006; evidência física em aparelhos e condições reais de campo continua reservada à OE-007.007.